...
from .lib import param  # local import
...
class VboxConfig():
    ...
    def batch_cleanup(self, sub_str):
        ...
        for each in vm_list:
            if sub_str in each:
                vbox = Vbox(basename=each, mode=param.VboxMode.use)
                vbox.is_clone = True
...
class Vbox():
    ...
    def __init__(self, basename="ubuntu-lts-base",
                 clonename="testvm", mode=param.VboxMode.clone,
        ...
        if not isinstance(mode, param.VboxMode):
            raise TypeError("mode must be of type VboxMode")

        self.vb = virtualbox.VirtualBox()

        if mode == param.VboxMode.clone:
    ...
    @check_stopped
    def start(self, session_type=param.SessionType.headless, wait=True):
        """start a machine
        ...
        """
        if not isinstance(session_type, param.SessionType):
    ...
    @check_running
    def stop(self, stop_mode=param.StopMode.shutdown, confirm=param.StopConfirm.none,
             wait=True, snap_name = ""):
        ...
        elif stop_mode is param.StopMode.poweroff:
        ...
        elif stop_mode is param.StopMode.save_state:
    ...
    @check_stopped
    def export(self, path="/tmp/disk.vdi", controller=param.ControllerType.SATA,
        ...
        if not isinstance(controller, param.ControllerType):
    ...
    def mount_cd(self, path, remove_image=False):
        ...
        self.medium = self.vb.open_medium(path,
                                          ...
                                          False)
        self.session.machine.mount_medium(param.ControllerType.IDE.name, 1, 0,
                                          self.medium, True)
    ...
    def cleanup_and_delete(self, ignore_errors=True, rm_clone=True, wait=True):
        ...
        if self.is_clone and rm_clone:
            ...
            hdd = self.session.machine.get_medium(param.ControllerType.SATA.name,
                                                  0, 0)
